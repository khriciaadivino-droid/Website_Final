 ...
 */
final class Version20260516173842 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $hasOrderQuantity = $hasOrders && $this->hasColumn($schemaManager, 'orders', 'quantity');
        ...
        $hasPetOfTheMonth = $hasPetProfileManagement && $this->hasColumn($schemaManager, 'pet_profile_management', 'is_pet_of_the_month');
        ...
        if ($hasOrders && !$hasOrderQuantity) {
            $this->addSql('ALTER TABLE orders ADD quantity INT NOT NULL');
        }

        if ($hasPetProfileManagement) {
            $columnsToAdd = [];
            if (!$hasOwnerId) {
                $columnsToAdd[] = 'ADD owner_id INT DEFAULT NULL';
            }
            if (!$hasPetOfTheMonth) {
                $columnsToAdd[] = 'ADD is_pet_of_the_month TINYINT(1) DEFAULT 0 NOT NULL';
            }
            if ($columnsToAdd !== []) {
