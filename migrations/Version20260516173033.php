 ...
 */
final class Version20260516173033 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($schemaManager->tablesExist(['productss'])) {
            ...
            $hasCategory = array_key_exists('category', $productssColumns);
            $hasCategoryId = array_key_exists('category_id', $productssColumns);
            $hasCreatedById = array_key_exists('created_by_id', $productssColumns);
            $hasImagefilename = array_key_exists('imagefilename', $productssColumns);
            $hasQuantity = array_key_exists('quantity', $productssColumns);

            $columnsToAdd = [];
            if (!$hasCategoryId) $columnsToAdd[] = 'ADD category_id INT DEFAULT NULL';
            if (!$hasCreatedById) $columnsToAdd[] = 'ADD created_by_id INT DEFAULT NULL';
            if (!$hasImagefilename) $columnsToAdd[] = 'ADD imagefilename VARCHAR(255) DEFAULT NULL';
            if (!$hasQuantity) $columnsToAdd[] = 'ADD quantity INT NOT NULL';

            $sql = 'ALTER TABLE productss ' . implode(', ', $columnsToAdd);
            if ($hasCategory) {
                $sql .= ', DROP category';
            }

            if (!empty($columnsToAdd) || $hasCategory) {
                $this->addSql($sql);
            }
        }
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('ALTER TABLE productss ADD category VARCHAR(100) NOT NULL, DROP category_id, DROP created_by_id, DROP imagefilename, DROP quantity');
    }
}
