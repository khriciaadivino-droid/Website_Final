 ...
 */
final class Version20260516172621 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($schemaManager->tablesExist(['productss'])) {
            ...
            $clauses = [];
            if (!$hasCategoryId) $clauses[] = 'ADD category_id INT DEFAULT NULL';
            if (!$hasCreatedById) $clauses[] = 'ADD created_by_id INT DEFAULT NULL';
            if (!$hasImagefilename) $clauses[] = 'ADD imagefilename VARCHAR(255) DEFAULT NULL';
            if (!$hasQuantity) $clauses[] = 'ADD quantity INT NOT NULL';
            if ($hasCategory) $clauses[] = 'DROP category';

            if (!empty($clauses)) {
                $sql = 'ALTER TABLE productss ' . implode(', ', $clauses);
                $this->addSql($sql);
            }
        }
