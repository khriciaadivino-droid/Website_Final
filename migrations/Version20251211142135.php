 ...
 */
final class Version20251211142135 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE IF NOT EXISTS shipment (id INT AUTO_INCREMENT NOT NULL, shipping_zone_id INT NOT NULL, courier_id INT NOT NULL, tracking_number VARCHAR(100) NOT NULL, customer_name VARCHAR(255) NOT NULL, customer_email VARCHAR(255) DEFAULT NULL, customer_phone VARCHAR(50) DEFAULT NULL, delivery_address LONGTEXT NOT NULL, status VARCHAR(50) NOT NULL, created_at DATETIME NOT NULL, shipped_at DATETIME DEFAULT NULL, delivered_at DATETIME DEFAULT NULL, estimated_delivery DATETIME DEFAULT NULL, shipping_cost NUMERIC(10, 2) NOT NULL, weight NUMERIC(10, 2) DEFAULT NULL, items LONGTEXT DEFAULT NULL, notes LONGTEXT DEFAULT NULL, UNIQUE INDEX UNIQ_2CB20DC3E1C9C18 (tracking_number), INDEX IDX_2CB20DC7964396F (shipping_zone_id), INDEX IDX_2CB20DCE3D8151C (courier_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $shipmentForeignKeys = $schemaManager->tablesExist(['shipment'])
            ? $schemaManager->listTableForeignKeys('shipment')
            : [];
        $hasShippingZoneFk = false;
        $hasCourierFk = false;
        foreach ($shipmentForeignKeys as $foreignKey) {
            if ($foreignKey->getName() === 'FK_2CB20DC7964396F') {
                $hasShippingZoneFk = true;
            }
            if ($foreignKey->getName() === 'FK_2CB20DCE3D8151C') {
                $hasCourierFk = true;
            }
        }
        if (!$hasShippingZoneFk) {
        ...
        $shipmentTrackingForeignKeys = $schemaManager->tablesExist(['shipment_tracking'])
            ? $schemaManager->listTableForeignKeys('shipment_tracking')
            : [];
        $hasShipmentTrackingFk = false;
        foreach ($shipmentTrackingForeignKeys as $foreignKey) {
            if ($foreignKey->getName() === 'FK_E2B9D7D7BE036FC') {
                $hasShipmentTrackingFk = true;
                break;
            }
        }
        if (!$hasShipmentTrackingFk) {
